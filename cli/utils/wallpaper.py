...
from cns.utils.colourfulness import get_variant
...
from cns.utils.scheme import Scheme, get_scheme
from cns.utils.theme import apply_colours


def _get_monitors() -> list[dict[str, int]]:
    try:
        monitors_json = subprocess.check_output(
            ["qs", "-c", "caelestia-niri-shell", "ipc", "call", "screens", "list"],
            text=True, stderr=subprocess.DEVNULL
        )
        return json.loads(monitors_json)
    except (subprocess.CalledProcessError, json.JSONDecodeError, FileNotFoundError):
        pass

    try:
        wlr_output = subprocess.check_output(
        ...
        for line in wlr_output.splitlines():
            line = line.strip()
            if " px, " in line:
                dims = line.split(" px, ")[0].split("x")
                if len(dims) == 2 and dims[0].isdigit() and dims[1].isdigit():
                    monitors.append({"width": int(dims[0]), "height": int(dims[1])})
        if monitors:
            return monitors
