...
from cns.utils.paths import atomic_dump, scheme_data_dir, scheme_path
...
def read_colours_from_file(path: Path) -> dict[str, str]:
    return {k.strip(): v.strip() for k, v in (line.split(None, 1) for line in path.read_text().splitlines() if line)}
