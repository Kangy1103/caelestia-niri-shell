...
from cns.utils.paths import config_dir


def print_version() -> None:
    if shutil.which("pacman"):
        print("Packages:")
        pkgs = ["cns-shell", "cns-cli", "cns-meta"]
    ...
    print()
    try:
        cns_dir = (config_dir / "caelestia-niri-shell").resolve()
        cns_ver = subprocess.check_output(
            ["git", "--git-dir", cns_dir / ".git", "rev-list", "--format=%B", "--max-count=1", "HEAD"], text=True
        )
        print("Caelestia:")
        print("    Last commit:", cns_ver.split()[1])
        print("    Commit message:", *cns_ver.splitlines()[1:])
    except subprocess.CalledProcessError:
        print("Caelestia: not installed")

    print()
    try:
        shell_ver = subprocess.check_output(["/usr/lib/caelestia-niri-shell/version", "-s"], text=True).strip()
        print("Shell:")
        print("    ", shell_ver)
    ...
    local_shell_dir = config_dir / "quickshell/caelestia-niri-shell"
