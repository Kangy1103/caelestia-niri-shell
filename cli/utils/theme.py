...
from cns.utils.colour import get_dynamic_colours
from cns.utils.logging import log_exception
from cns.utils.paths import (
    c_state_dir,
    config_dir,
    data_dir,
    templates_dir,
...
def apply_discord(scss: str) -> None:
    ...
    for client in "Equicord", "Vencord", "BetterDiscord", "equibop", "vesktop", "legcord":
        write_file(config_dir / client / "themes/cns.theme.css", conf)
...
def apply_pandora(colours: dict[str, str], mode: str) -> None:
    ...
    write_file(data_dir / "PandoraLauncher/themes/cns.json", template)


@log_exception
def apply_spicetify(colours: dict[str, str], mode: str) -> None:
    template = gen_replace(colours, templates_dir / f"spicetify-{mode}.ini")
    write_file(config_dir / "spicetify/Themes/cns/color.ini", template)
...
def apply_btop(colours: dict[str, str]) -> None:
    ...
    write_file(config_dir / "btop/themes/cns.theme", template)
...
def apply_qt(colours: dict[str, str], mode: str, icon_theme: str | None = None) -> None:
    ...
    write_file(config_dir / "qtengine/cns.colors", colours)
...
def apply_warp(colours: dict[str, str], mode: str) -> None:
    ...
    write_file(data_dir / "warp-terminal/themes/cns.yaml", template)
...
def apply_chromium(colours: dict[str, str]) -> None:
    ...
    for cmd, policy_dir in browsers:
        ...
        subprocess.run(
            ["sudo", "-n", "tee", str(policy_dir / "cns.json")],
