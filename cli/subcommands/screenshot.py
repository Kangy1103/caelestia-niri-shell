...
from cns.utils.notify import notify
from cns.utils.paths import screenshots_cache_dir, screenshots_dir
...
class Command:
    ...
    def region(self) -> None:
        if self.args.region == "slurp":
            subprocess.run(
                ["qs", "-c", "caelestia-niri-shell", "ipc", "call", "picker", "openFreeze" if self.args.freeze else "open"]
            )
        else:
    ...
    def fullscreen(self) -> None:
        ...
        dest = screenshots_cache_dir / datetime.now().strftime("%Y%m%d%H%M%S%f")
